...
export const ResumeDropzone = ({
  ...
  const handleAuthCheckAndProceed = (callback: () => void) => {
    if (!user) {
  ...
  const onDrop = useCallback(
    (acceptedFiles: File[]) => {
      handleAuthCheckAndProceed(async () => {
        const file = acceptedFiles[0];
        if (!file) return;
        ...
        try {
          const response = await fetch(
            "http://127.0.0.1:5001/api/parse-resume",
            {
              method: "POST",
              body: formData,
            }
    ...
    [onParsed, user, signInWithGoogle]
  );
  ...
  return (
    <GlassCard isDark={isDark} padding={false}>
      <div {...getRootProps()} className="outline-none">
        <motion.div
          onClick={() =>
            handleAuthCheckAndProceed(() => inputRef.current?.click())
          }
          whileHover={!isLoading ? { scale: 1.01 } : {}}
          className={`p-12 border-2 border-dashed rounded-2xl cursor-pointer transition-all duration-300 ${
            isDragActive
        ...
        >
          <input
            {...getInputProps({ ref: inputRef, style: { display: "none" } })}
          />
            ...
            <div>
              ...
              >
                {isLoading
                  ...
                  : user
                  ? "Drag and drop or click to select a PDF file"
                  : "Please sign in to build your AI assistant"}
              </p>
            </div>
          </div>
        </motion.div>
